...
const AddProperty = () => {
  ...
  const onSubmit = async (data: PropertyFormData) => {
    try {
      ...
      // Automatically create units based on unit count
      const numUnits = parseInt(data.units);
      if (numUnits > 1) {
        // Create numbered units for properties with multiple units
        const unitsToCreate = Array.from({ length: numUnits }, (_, i) => ({
      ...
      toast({
        ...
        description: `${data.name} has been successfully added to your portfolio.`,
      });
      navigate("/properties");
    } catch (error: any) {
      toast({
      ...
      <div className="p-8 max-w-4xl mx-auto">
        ...
        <Card>
          <CardContent className="p-8">
            <h1 className="text-2xl font-bold text-foreground mb-6">Create New Property</h1>
